...
class TestIsInOne(unittest.TestCase):
  ...
  def test_0(self):
    ...
    string = 'Westlife'
    self.assertEqual(is_in_one(list_1, list_2, string), True)
    
  def test_1(self):
    """A false case where the string is in both of the lists"""
    list_1 = ['Westlife', 'Akon', 'Queen']
    list_2 = ['Avril Lavigne', 'Lady Gaga', 'Westlife']
    string = 'Westlife'
    self.assertEqual(is_in_one(list_1, list_2, string), False)
  ...
  def test_7(self):
    ...
    with self.assertRaises(AssertionError):
      is_in_one(list_1, list_2, string)
